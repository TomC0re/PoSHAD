$Theme = "Разблокирована учетная запись пользователя"
$Subject = "Разблокирован пользователь"
...
$Info = Get-WinEvent -FilterHashtable @{LogName="Security";ID=4767} | Select TimeCreated,@{n="User";e={([xml]$_.ToXml()).Event.EventData.Data | ? {$_.Name -eq "TargetUserName"} | %{$_.'#text'}}},@{n="Initiator";e={([xml]$_.ToXml()).Event.EventData.Data | ? {$_.Name -eq "SubjectUserName"}| %{$_.'#text'}}} | select-object -first 1
