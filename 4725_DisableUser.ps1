$Theme = "Отключена учетная запись пользователя"
$Subject = "Отключен пользователь"
...
$Info = Get-WinEvent -FilterHashtable @{LogName="Security";ID=4725} | Select TimeCreated,@{n="User";e={([xml]$_.ToXml()).Event.EventData.Data | ? {$_.Name -eq "TargetUserName"} | %{$_.'#text'}}},@{n="Initiator";e={([xml]$_.ToXml()).Event.EventData.Data | ? {$_.Name -eq "SubjectUserName"}| %{$_.'#text'}}} | select-object -first 1
