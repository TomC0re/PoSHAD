$Theme = "Учетная запись пользователя изменена"
...
$Info = Get-WinEvent -FilterHashtable @{LogName="Security";ID=4738} | Select TimeCreated,`
    @{n="User";e={([xml]$_.ToXml()).Event.EventData.Data | ? {$_.Name -eq "TargetUserName"}| %{$_.'#text'}}},`
    ...
    @{n="LogonHours";e={([xml]$_.ToXml()).Event.EventData.Data | ? {$_.Name -eq "LogonHours"}| %{$_.'#text'}}} | select-object -first 1
