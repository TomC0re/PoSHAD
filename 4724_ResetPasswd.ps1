$Theme = "Попытка сбросить пароль учетной записи"
$Subject = "Попытка сбросить пароль"
...
$Info = Get-WinEvent -FilterHashtable @{LogName="Security";ID=4724} | Select TimeCreated,@{n="User";e={([xml]$_.ToXml()).Event.EventData.Data | ? {$_.Name -eq "TargetUserName"} | %{$_.'#text'}}},@{n="Initiator";e={([xml]$_.ToXml()).Event.EventData.Data | ? {$_.Name -eq "SubjectUserName"}| %{$_.'#text'}}} | select-object -first 1
